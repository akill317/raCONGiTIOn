	...
	#pragma shader_feature ETC1_EXTERNAL_ALPHA
		...
		void vert(inout appdata_full v, out Input o)
		{
			...
			float4 offset = float4(
				tex2Dlod(_NoiseTex, float4(v.vertex.z / _DistortionSpreader + _Time[1]/ _VertexOffsetAmount, 0, 0, 0)).r - 0.5,
				tex2Dlod(_NoiseTex, float4(0, v.vertex.x / _DistortionSpreader + _Time[1] / _VertexOffsetAmount, 0, 0)).r - 0.5,
                tex2Dlod(_NoiseTex, float4(0, 0, v.vertex.y / _DistortionSpreader + _Time[1] / _VertexOffsetAmount, 0)).r - 0.5,
				0
				);
			v.vertex += offset ;
